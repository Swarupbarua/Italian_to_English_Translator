...
def maskOrig(fullTxt, k1, k2, all_recog_text, all_orig_text,all_bx, orig, boundary, \
            encoder, onestepdecoder, inp_lang, targ_lang, max_length_inp, max_length_targ ):
	...
	for k, textRecognized in enumerate(all_recog_text):
		...
		if (textRecognized[-1] not in ['.','?','!']):
			...
			if last_ln:
				if k!= 0:
					fullTxt += ' '+ textRecognized
				orig = maskOrig(fullTxt, prek, k,all_recog_text, all_orig_text,all_bx, orig, boundary, \
                                encoder, onestepdecoder, inp_lang, targ_lang, max_length_inp, max_length_targ)
				fullTxt = ''
				prek = k+1
...
def about():
	st.write(
		'''
		Here is the pipelie followed to translate a document.

			1. Text detection using EAST (An Efficient and Accurate Scene Text Detector).
			2. Text Recognition using pytesseract.
			3. Text Translation using Attention mechanism.
			4. Re structuring translated Text.
Read more : https://github.com/Swarupbarua/Italian_to_English_Translator
		''')


def main():
    st.title("Document Transalation App :sunglasses: ")
    st.write("**Using Attention, EAST and Tesseract.**")

    activities = ["Home", "About"]
    choice = st.sidebar.selectbox("Pick some document", activities)
    ...
    if choice == "Home":
    	...
    	if image_file is not None:
    		...
    		if st.button("Process"):
    			...
    			result_img, duration = detect(image=image)
    			st.image(result_img, use_column_width = True)
    			st.success("Document Trasalated\nProcessing Time {} ".format(duration))
...
if __name__ == "__main__":
	...
	inp_lang, targ_lang, max_length_inp, max_length_targ, inp_vocab_size, \
        out_vocab_size, _, _, inp_embedding_dim,out_embedding_dim, \
        inp_lstm_size, dec_unit, att_units = pickle.load(open("all_data_att_final.pkl", 'rb'))
